...
void ADestructedBrick::setItemOptions(const FItemOptions& Item)
{
    Item_Options = Item;
    UMasterMakerGameInstance* instance = Cast<UMasterMakerGameInstance>(GetGameInstance());
    TArray<UStaticMesh*> assets = instance->Destructed_Meshes[Item.Name];
    if (Item.CategoryName ==  Mesh_Categories.at(MeshCategories::characters)) {
        setCubeManCharacterMesh(assets);
    }
    else if (Item.CategoryName == Mesh_Categories.at(MeshCategories::vehicles)) {
    ...
    setColor(Item_Options.Color);
}

void ADestructedBrick::setColor(FLinearColor ColorVec)
{
...
void ADestructedBrick::setImpactPointAndVelocity(const FVector& ImpactPoint, const FVector& Velocity)
{
    ...
    {
        if (Item_Options.CategoryName == Mesh_Categories.at(MeshCategories::characters)) {
