...
void AWeaponBrick::Tick(float DeltaTime)
{
    ...
    if (Owner_Vehicle) {
        if (Owner_Vehicle->carHasPassenger() && Type_ == WeaponType::fire)
        {

            // Get the player controller

            // Get the player's camera rotation
            FRotator CameraRotation = Owner_Vehicle->Camera->GetComponentRotation();
...
{
    if (Owner_Vehicle)
        Owner_Vehicle->removeWeaponToInventory(Weapon_Index);
...
void AWeaponBrick::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    else if (Cast<AVehicleBase>(OtherActor) && !(Cast<AVehicleBase>(OtherActor) == Owner_Vehicle)) {
        UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), Particle_Effect, GetActorLocation());
...
{
    if (Item_Options.Name.Find(MELEE_WEAPON_APPENDIX) >= 0)
        Type_ = WeaponType::melee;
    else if (Item_Options.Name.Find(FIRE_WEAPON_APPENDIX) >= 0)
        Type_ = WeaponType::fire;
    else if (Item_Options.Name.Find(EXPLOSIVE_WEAPON_APPENDIX) >= 0)
        Type_ = WeaponType::explosive;
    else if (Item_Options.Name.Find(CLUSTER_WEAPON_APPENDIX) >= 0)
...
void AWeaponBrick::applyForce(const FVector& ImpulseForce, const FVector& TorqueForce, const FVector& ForceLocation)
{
    ...
    if (Item_Options.Name.Find(CLUSTER_WEAPON_APPENDIX) >= 0) {
        AWeaponBrick* bombs;
        FVector brick_loc = Brick->GetComponentLocation();
        Brick->DestroyComponent();
        FItemOptions item; 
        item.Name = "Bomb_explosive_weapon";
        item.CategoryName = Mesh_Categories.at(MeshCategories::weapons);

        bombs = Cast<UMasterMakerGameInstance>(GetWorld()->GetGameInstance())->spawnWeapon(item, brick_loc + FVector(20, 0, 0), FRotator(0, 0, 0));
        bombs->scaleBomb(0.3);
        ...
        bombs = Cast<UMasterMakerGameInstance>(GetWorld()->GetGameInstance())->spawnWeapon(item, brick_loc + FVector(0, 20, 0), FRotator(0, 0, 0));
        bombs->scaleBomb(0.3);
        bombs->applyForce(ImpulseForce, TorqueForce, brick_loc);
...
void AWeaponBrick::fire()
{
    ...
    if (Owner_Vehicle && Type_ == WeaponType::fire) {
