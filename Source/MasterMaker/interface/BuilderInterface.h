...
class MASTERMAKER_API IBuilderInterface
{
	...
	void CalculateOBBCorners(const FTransform& Transform, const FVector& Extents, TArray<FVector>& OutCorners);
	float GetOBBOverlapPercentage(UStaticMeshComponent* GhostComponent);
	bool DoOBBsRoughlyIntersect(const TArray<FVector>& Box1, const TArray<FVector>& Box2);
...
public:
	...
	void plugObject(ABrick* GrabbedBrick, IPlugInterface* TargetPlugable, const FVector& TargetLocation,
	...
	void checkGhostItemOverlap();
	float getOverlapPercentage(UStaticMeshComponent* GhostComponent);
