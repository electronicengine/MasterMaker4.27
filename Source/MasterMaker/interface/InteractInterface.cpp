...
int IInteractInterface::buyBrick(const FVector& Location, const FItemOptions& Item, int Id)
{
    FActorSpawnParameters params;
    params.Owner = Cast<ACharacter>(this);

    if (Cast<ISlingInterface>(this)->Sling_Mode_Enable && Item.Name.Find("Sling") >= 0)
        return 0;

    AActor* spawned_object = Game_Instance->spawnItem(Location, FRotator(0, 0, 0), Item, Location, params);
...
void IInteractInterface::interactNearby(AActor* OtherActor)
{
    ...
    else
    {
        ...
        if (brick != nullptr) {
            ...
            if (brick->Owner_Vehicle != nullptr) {
                Object_NearBy = brick->Owner_Vehicle;
            }
            else {
                Object_NearBy = brick;
            }
