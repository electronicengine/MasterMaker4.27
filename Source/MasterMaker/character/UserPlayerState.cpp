...
{
    _Char = Cast<AUserCharacter>(GetPawn());
    FItemOptions item = _Char->Game_Instance->Selected_Item;
    FVector spawn_location = _Char->Aim_Impact_Point;

    if (_Char->Sling_Mode_Enable) {
        buyBrickOnServer(_Char->Projectile_Location, _Char->Game_Instance->Selected_Item);
    }
    else {

        if (_Char->Game_Instance->Selected_Item.CategoryName == Mesh_Categories.at(MeshCategories::mechanics)) {
            spawn_location = _Char->GetMesh()->GetSocketLocation(TEXT("grab_socket"));
        }

        if (_Char->Grabbable_Brick == nullptr && _Char->Game_Instance) {

            buyBrickOnServer(spawn_location, _Char->Game_Instance->Selected_Item);
        }
    }
...
void AUserPlayerState::plugObject()
{
    ...
    if (_Char->Grabbable_Brick != nullptr && _Char->Building && _Char->Brick_Plugable)
    {
        if (_Char->Ghost->IsVisible())
        {
            ...
            plugObjectOnServer(_Char->Aim_Impact_Point, grabbed_id, target_id, target_loc,target_rot,
...
void AUserPlayerState::buyBrickOnServer_Implementation(const FVector& Location, const FItemOptions& Item)
{
    ...
    if (GetNetMode() == ENetMode::NM_ListenServer || GetNetMode() == ENetMode::NM_Standalone || GetNetMode() == ENetMode::NM_DedicatedServer) {
        ...
        if (Item.SubCategoryName != Mesh_Sub_Categories.at(vehicle_skeleton) && Item.CategoryName != Mesh_Categories.at(characters)) {
            _Char = Cast<AUserCharacter>(GetPawn());
            _Char->buyBrick(Location, Item, id);
            buyBrickOnMulticast(Location, Item, id);
        }
        else {
            FTimerHandle TimerHandle;
            UMasterMakerGameInstance* game_instance = Cast<UMasterMakerGameInstance>(GetWorld()->GetGameInstance());
            AActor* spawned_object = game_instance->spawnItem(Location, FRotator(0, 0, 0), Item, Location);

            if (Cast<IBreakableInterface>(spawned_object))
                Cast<IBreakableInterface>(spawned_object)->setId(id);

            GetWorld()->GetTimerManager().SetTimer(TimerHandle, [this, Location, Item, id]()
                {
                    buyBrickOnMulticast(Location, Item, id);
                }, 0.5, false);
        }
...
void AUserPlayerState::buyBrickOnMulticast_Implementation(const FVector& Location, const FItemOptions& Item, int Id)
{
    _Char = Cast<AUserCharacter>(GetPawn());
...
void AUserPlayerState::plugObjectOnServer_Implementation(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
    const FRotator& TargetRotation,  const FRotator& OffsetRotation, const FVector& OffsetLocation)
{
    _Char = Cast<AUserCharacter>(GetPawn());
    ...
    if (GetNetMode() == ENetMode::NM_ListenServer || GetNetMode() == ENetMode::NM_Standalone || GetNetMode() == ENetMode::NM_DedicatedServer) {
        ...
        if (grabbed)
            _Char->plugObject(grabbed, pluggable, TargetLocation, TargetRotation, OffsetRotation, OffsetLocation);

        plugObjectOnMulticast(TargetPlugableLocation, GrabbedId, TargetId, TargetLocation,TargetRotation, OffsetRotation, OffsetLocation);

    }

}

void AUserPlayerState::plugObjectOnMulticast_Implementation(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
    const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation)
{
    _Char = Cast<AUserCharacter>(GetPawn());
