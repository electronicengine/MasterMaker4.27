...
public:
	...
	UFUNCTION(Server, Reliable, WithValidation)
	void buyBrickOnServer(const FVector& Location, const FItemOptions& Item);
	bool buyBrickOnServer_Validate(const FVector& Location, const FItemOptions& Item) { return true; }
	void buyBrickOnServer_Implementation(const FVector& Location, const FItemOptions& Item);

	UFUNCTION(NetMulticast, Reliable, WithValidation)
	void buyBrickOnMulticast(const FVector& Location, const FItemOptions& Item, int Id);
	bool buyBrickOnMulticast_Validate(const FVector& Location, const FItemOptions& Item, int Id) { return true; }
	void buyBrickOnMulticast_Implementation(const FVector& Location, const FItemOptions& Item, int Id);

	UFUNCTION(Server, Reliable, WithValidation)
	void plugObjectOnServer(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
			const FRotator& TargetRotation,  const FRotator& OffsetRotation, const FVector& OffsetLocation);
	bool plugObjectOnServer_Validate(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
		const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation) {
		return true;
	}
	void plugObjectOnServer_Implementation(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
		const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation);

	UFUNCTION(NetMulticast, Reliable, WithValidation)
		void plugObjectOnMulticast(const FVector& TargetPlugableLocation,int GrabbedId, int TargetId, const FVector& TargetLocation,
			const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation);
	bool plugObjectOnMulticast_Validate(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
		const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation) {
		return true;
	}
	void plugObjectOnMulticast_Implementation(const FVector& TargetPlugableLocation, int GrabbedId, int TargetId, const FVector& TargetLocation,
		const FRotator& TargetRotation, const FRotator& OffsetRotation, const FVector& OffsetLocation);
